...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-100 transition-all duration-500 flex justify-center ${scrolled ? "pt-4" : "pt-0"}`}>
            ...
            {/* Mobile Sidebar */}
            <div
                className={`fixed top-0 right-0 h-full w-[240px] sm:w-[300px] bg-[#0a0a0a] z-108 lg:hidden transition-transform duration-500 ease-in-out shadow-2xl ${isOpen ? "translate-x-0" : "translate-x-full"}`}
            >
                <div className="flex flex-col h-full p-6">
                    <div className="flex justify-between items-center mb-8">
                        <div className="flex items-center space-x-3">
                            <div className="w-8 h-8 bg-[#F5D76E] rounded flex items-center justify-center text-black font-bold text-base">M</div>
                            <span className="text-base tracking-tight text-white leading-tight">
                                <span className="font-light">Mesum</span><span className="font-bold text-[#F5D76E]">Ali</span>
                            </span>
                        </div>
                        ...
                        {navLinks.map((link) => (
                            <li key={link.to}>
                                <Link
                                    ...
                                    className="text-base text-[#E5E5E5]/80 hover:text-[#F5D76E] hover:bg-white/5 font-medium transition-all duration-300 block py-2.5 px-4 rounded-xl cursor-pointer"
                                    onClick={() => setIsOpen(false)}
                                >
                                    {link.name}
                                </Link>
                    ...
                    <div className="mt-auto space-y-6">
                        <div className="flex items-center space-x-5 px-4">
                            {[
                                { icon: <FaLinkedin size={18} />, url: "https://www.linkedin.com/in/mesumali-dev/" },
                                { icon: <FaTwitter size={18} />, url: "https://x.com/mesumali_dev" },
                                { icon: <FaGithub size={18} />, url: "https://github.com/mesumali-dev/" }
                            ].map((social, index) => (
                        ...
                        <a
                            href="/Resume.pdf"
                            download="Mesum_Ali_Resume.pdf"
                            className="group w-full flex items-center justify-center space-x-3 bg-[#F5D76E] hover:bg-[#d4b74e] text-black py-3 rounded-full font-bold text-sm shadow-lg shadow-[#F5D76E]/10 transition-all duration-300 transform hover:-translate-y-0.5"
                            onClick={() => setIsOpen(false)}
                        >
                            <span>Download CV</span>
                            <LuDownload size={14} className="group-hover:translate-y-0.5 transition-transform duration-300" />
                        </a>
                    </div>
                </div>
            </div>
